...
def build_export_dag(
    ...
    def export_blocks_command(execution_date, provider_uri, **kwargs):
        with TemporaryDirectory() as tempdir:
            ...
            copy_to_export_path(
                os.path.join(tempdir, "block.json"), export_path("block", execution_date)
            )

            copy_to_export_path(
                os.path.join(tempdir, "balance_update.json"), export_path("balance_update", execution_date)
            )

            for operation_type in OperationType.ALL:
                local_path = os.path.join(tempdir, f"{operation_type}_operation.json")
                remote_path = export_path(f"{operation_type}_operation", execution_date)
